...
@section('page-title', 'Conversation avec l’administrateur')
...
<div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-800 flex items-center justify-between gap-3">
        <div>
            ...
            <p class="text-slate-500 text-xs mt-0.5">Prestataire: {{ $conversation->provider->name }}</p>
        ...
        <span class="px-2 py-1 rounded-full text-xs {{ $conversation->status === 'closed' ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300' }}">
                    ...
                    @if($mine)
                        <div class="mt-2 flex flex-wrap gap-2">
                            <button type="button"
                                    ...
                                    data-edit-action="{{ route('provider.conversations.messages.update', [$conversation, $message]) }}"
                                    data-edit-body="{{ e($message->body) }}">
                                Modifier
                            </button>
                            <form method="POST" action="{{ route('provider.conversations.messages.delete', [$conversation, $message]) }}" onsubmit="return confirm('Supprimer ce message ?');">
                                @csrf
                                @method('DELETE')
                    ...
                    @if($message->attachments->isNotEmpty())
                        <div class="mt-2 flex flex-wrap gap-2">
                            @foreach($message->attachments as $attachment)
                                ...
                                <a href="{{ route('provider.conversations.attachments.download', [$conversation, $attachment]) }}"
                                ...
                                @if($isInline)
                                    @if(str_starts_with((string) $attachment->mime_type, 'image/'))
                                        <img src="{{ route('provider.conversations.attachments.preview', [$conversation, $attachment]) }}?thumb=1" alt="{{ $attachment->file_name }}" class="mt-2 max-h-48 rounded-lg border border-white/20">
                                    @elseif($attachment->mime_type === 'application/pdf')
                                        <iframe src="{{ route('provider.conversations.attachments.preview', [$conversation, $attachment]) }}" class="mt-2 h-56 w-full rounded-lg border border-white/20"></iframe>
                                    @endif
                                @endif
                            @endforeach
                        </div>
    ...
    <div class="px-5 py-4 border-t border-slate-800">
        <form method="POST" action="{{ route('provider.conversations.reply', $conversation) }}" enctype="multipart/form-data" class="space-y-3">
            @csrf
            <textarea name="message" rows="3" required
                      class="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-100"
                      placeholder="Écrire un message à l’administrateur..."></textarea>
            <div>
